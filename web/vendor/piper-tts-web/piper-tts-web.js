...
/** App-controlled speaker id for multi-speaker Piper models (library default was hardcoded 0). */
let _activeSpeakerId = 0;
/** Optional length_scale override (null = use model config). */
let _activeLengthScale = null;
function setActiveSpeakerId(id) {
  _activeSpeakerId = Number.isFinite(id) ? Number(id) : 0;
}
function setActiveLengthScale(scale) {
  _activeLengthScale = (scale == null || scale === "") ? null : Number(scale);
}
...
predictChunk_fn = async function(text) {
  ...
  const speakerId = _activeSpeakerId || 0;
  const noiseScale = __privateGet(this, _modelConfig).inference.noise_scale;
  const lengthScale = (_activeLengthScale != null && Number.isFinite(_activeLengthScale) ? _activeLengthScale : __privateGet(this, _modelConfig).inference.length_scale);
...
async function predict(config, callback) {
  if (config && typeof config.speakerId === "number") setActiveSpeakerId(config.speakerId);
  else if (config && config.speakerId != null && config.speakerId !== "") setActiveSpeakerId(Number(config.speakerId));
  if (config && "lengthScale" in config) setActiveLengthScale(config.lengthScale);
